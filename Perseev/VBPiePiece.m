...
- (void) pieceAngle:(float)angle start:(float)startAngle; {
    ...
    if (_accent) {
        ...
        [layer setFrame:CGRectMake(0, 0, 2, 2)];
        [layer setPosition:CGPointMake(center.x+_accentVector.x*size.width/3, center.y+_accentVector.y*size.height/3)];
    ...
    if (startAngle > -.01 && angle > -.01) {
        ...
        [self __angle:_angle];
        CGAffineTransform rotateTransform = CGAffineTransformMakeRotation(_angle);
        [layer setAffineTransform:rotateTransform];
        //layer.transform = CATransform3DMakeRotation(_angle, 0.0, 0.0, 1.0);
    }
}
