...
@property (nonatomic, retain) UIColor *CircleImage;
...
- (void) updateCharts {
    ...
    for (NSDictionary *object in _chartValues) {
        ...
        if ([object isKindOfClass:[NSDictionary class]]) {
            ...
            //data.CircleImage = [UIImage imageNamed:[dict objectForKey:@"color"]];
            
            NSLog(@"--- circle image --%@",[dict objectForKey:@"color"]);
    ...
    for (VBPiePieceData *data in _chartsData) {
      ...
      /**  UIGraphicsBeginImageContext(piece.frame.size);
        [[UIImage imageNamed:@"sehwag-mi300.jpg"] drawInRect:piece.bounds];
        UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
        UIGraphicsEndImageContext();
        
        //NSLog(@"piece.frame ====== %f ======= %f ======= %f ====== %f",piece.frame.origin.x,piece.frame.origin.y,piece.frame.size.height,piece.frame.size.width);
        
       //
        piece.fillColor = [UIColor colorWithPatternImage:image].CGColor;
...
- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (CGPointDistanceBetweenTwoPoints(point, _touchBegan) < 5) {
        ...
        //_hitLayer.borderColor = [UIColor redColor].CGColor;
        //_hitLayer.borderWidth = 1.0f;
